...
from rewards.rewards_utils import display_all_programs_info, build_request_url_with_projection_query, get_dt_combos_from_zone, \
    create_product_list_from_zone, generate_combos_information, generate_categories_information, generate_terms_information, \
    get_id_rewards
...
def update_dt_combos_rewards(zone, environment):
    ...
    update_program_combos = input(text.Yellow + '\n- Do you want to update the program configuration to include "{}" redeem products? y/N: '
...
def missing_dt_combos_to_include(program_dt_combos, zone_dt_combos):
    ...
    print(text.Yellow + '\n- Found "{}" redeem products missing in the program configuration.'.format(str(len(missing_combos_list))))
