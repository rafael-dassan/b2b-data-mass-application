    ...
    if len(items) == 1:
        ...
        for i in range(len(items)):
            if items[i]["quantity"] > 1:
                ...
                update_value_to_json(
                    order_data[0], "items[" + str(i) + "].quantity", item_qtd
                )
                update_value_to_json(
                    order_data[0], "items[" + str(i) + "].subtotal", item_subtotal
                )
                update_value_to_json(
                    order_data[0], "items[" + str(i) + "].total", item_total
                )
...
def request_get_order_by_date_updated(zone, environment, account_id, order_prefix):
    ...
    request_url = get_microservice_base_url(
        environment
    ) + "/order-service/v1?updatedSince={}&accountId={}&sort={}".format(
        updated_since, account_id, "DESC"
    )
